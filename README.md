...
### 🎪 全屏演示模式
按方向键翻页，`Esc` 退出，`N` 键切换演讲者备注——就像真正的演示软件一样。支持 Fade / Slide / Zoom 切换动画，并可继承 PPT 中识别出的切换与入场节奏。
...
### 🎬 PPT 动画智能映射
上传的 PPTX 中，**页面切换（transition）**与**元素入场（entrance）**动画会被自动识别，并映射到 Web Deck 的动效词汇（`fade` / `slide-up` / `scale` / `stagger`）。无需手动配置即可在网页演示中还原 PPT 的节奏感——映射结果作为 deck 级的 `motion` 与逐章节的 `sectionMotion` 应用，并可在编辑器「动画面板」中微调。复杂时间线与触发器为近似还原。

### 🛡️ 工程质量基线
内置 GitHub Actions CI（lint / build / 测试覆盖率门禁）与 ESLint 规范，配合 Vitest 覆盖率门禁，保证每次提交的稳定性，为后续版本迭代兜底。
